...
interface ApprovalQueueProps {
  logs: WorkerLog[];
  onApprove: (id: string, adjustedStart?: Date, adjustedEnd?: Date, reason?: string) => void;
...
const LogApprovalCard: React.FC<{ 
    log: WorkerLog; 
    onApprove: (id: string, start?: Date, end?: Date, reason?: string) => void; 
    onReject: (id: string) => void; 
...
const HistoryLogCard: React.FC<{ log: WorkerLog }> = ({ log }) => {
    // Check if adjusted
        ...
        <div className="p-4 bg-white border-b border-gray-100 hover:bg-gray-50 transition-colors">
             <div className="flex flex-col md:flex-row gap-4">
                <div className="w-full md:w-48 flex-shrink-0">
                     <p className="font-semibold text-gray-900 text-sm">{log.workerName}</p>
                     <p className="text-xs text-gray-500">{log.role}</p>
                     <div className={`mt-2 inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold uppercase border ${log.status === LogStatus.APPROVED ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-700 border-red-200'}`}>
                         {log.status}
                     </div>
                </div>
